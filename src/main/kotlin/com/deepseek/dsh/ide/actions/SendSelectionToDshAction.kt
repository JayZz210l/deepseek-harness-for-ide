...
import com.deepseek.dsh.ide.ui.DshBrowserBridge
...
/**
 * Editor context-menu action: appends the selected text (or the current line
 * when nothing is selected) to the currently visible Harness composer.
 *
 * Keeping submission in the browser is important: the browser owns both the
 * active session and blank-session creation.  It also lets the user add their
 * question before deliberately sending the combined draft.
 */
            ...
            ApplicationManager.getApplication().invokeLater {
                if (!project.isDisposed) {
                    ToolWindowManager.getInstance(project).getToolWindow("Deepseek Harness For IDE")?.show {
                        val appended = project.service<DshBrowserBridge>().appendToComposer(text)
                        if (appended) {
                            notify(project, DshBundle.message("dsh.notify.addedToComposer"), NotificationType.INFORMATION)
                        } else {
                            notify(project, DshBundle.message("dsh.notify.composerUnavailable"), NotificationType.WARNING)
                        }
                    }
                }
            }
